...
```{r}
chisq.test(train_data$FlightDelayStatus, train_data$ExpectedArrival, correct=FALSE)
```
...
train_rows <- createDataPartition(train_data$FlightDelayStatus, p = 0.90, list = F )
...
```{r}
std_method <- preProcess(pre_train[, !(names(pre_train) %in% c("FlightDelayStatus"))], method = c("center", "scale"))
...
```
## Access the auc score from the performance object: AUC:77.66
```{r}
...
```
## F1 score on Validation data-->78.43%  
...
#train_data$FlightDelayStatus<-as.factor(as.character(train_data$FlightDelayStatus))
mtry <- tuneRF(train_data[, 1:12], train_data$FlightDelayStatus, ntreeTry = 100, stepFactor = 1.5, improve = 0.01, trace = F, plot = T)
...
```{r}
trees = c(75,100,150,175,200,250)
